    ...
    with torch.no_grad():
        for step, (inputs, labels) in enumerate(val_dataloader):
            ...
            if int(top1.avg)>=niubi:
                niubi = int(top1.avg)
                torch.save(model.module.state_dict(), 'Fall_down_5_frame_5.13'+".pth.tar")
...
def main():
    ...
    val_dataloader = \
        DataLoader(
            VideoDataset_3_frames(mode='test'),
            batch_size=32, shuffle=True)
    # test_dataloader = \
    #     DataLoader(
    ...
    optimizer = optim.SGD(politics, lr=1e-3, momentum=0.9, weight_decay=5e-4)
