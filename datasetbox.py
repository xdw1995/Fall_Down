...
sys.path.insert(0, '/data/xudw/zzg_reid')
from xdw_actionrecognition import produce_frames
...
class VideoDataset_single(Dataset):
    def __init__(self, mode='train'):
        ...
        for i in f.readlines():
            ...
            if mode == 'train':
                if int(re.findall('(\d+)', list_path[-1])[0]) in [13, 14, 15, 16, 17]:
                    continue
...
class VideoDataset_3_frames(Dataset):
    ...
    def __init__(self, mode='train'):
        ...
        for i in f.readlines():
            ...
            if mode == 'train':
                ...
                if int(re.findall('(\d+)', list_path[-1])[0]) in [13, 14, 15, 16, 17]:
                    continue
                # # if '/data/xudw/Fall_Down_data/Subject5Activity4Trial1Camera2' in list_path[-1]:
                # #     continue
                # # if '/data/xudw/Fall_Down_data/Subject10Activity3Trial3Camera2' in list_path[-1]:
                # #     continue
                # # if '/data/xudw/Fall_Down_data/Subject8Activity4Trial1Camera2' in list_path[-1]:
                # #     continue
                # # if '/data/xudw/Fall_Down_data/Subject3Activity6Trial3Camera1' in list_path[-1]:
                # #     continue
                # # if '/data/xudw/Fall_Down_data/Subject10' in list_path[-1]:
                # #     continue
                # # if '/data/xudw/Fall_Down_data/Subject11Activity10' in list_path[-1]:
                # #     continue
                #
                # if '/data/xudw/Fall_Down_data/Subject11Activity10Trial3Camera1' in list_path[0]:
                #     a = a+1
                #     # import pdb
                #     # pdb.set_trace()
                #     # print(list_path[0]
                ...
                #     self.labels.append(self.dic[int(list_label[-1])])
                #
                # # elif '/data/xudw/Fall_Down_data/Subject11Activity1Trial1Camera1/2018-07-10T12_21_25.558481.png' in list_path[0]:
                # #     self.fname.append(list_path)
                # #     self.labels.append(self.dic[int(list_label[-1])])
                # elif '/data/xudw/Fall_Down_data/Subject11Activity1Trial1Camera1' in list_path[0]:
                #     self.fname.append(list_path)
                #     self.labels.append(self.dic[int(list_label[-1])])
                # elif '/data/xudw/Fall_Down_data/Subject11Activity1Trial1Camera1/2018-07-10T12_21_26.045811.png' in list_path[0]:
                #     self.fname.append(list_path)
                #     self.labels.append(self.dic[int(list_label[-1])])
                # # elif '/data/xudw/Fall_Down_data/Subject11Activity10Trial3Camera1/2018-07-10T10_55_56.727154.png' in list_path[0]:
                # #     self.fname.append(list_path)
                # #     self.labels.append(self.dic[int(list_label[-1])])
                # else:
                #     continue
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            for i in buffer:
                a = a + 1
                cv2.imwrite(','.join(self.fname[index][0].split('/')) + '.jpg', i)
                break
        ...
        if self.mode == 'test':
            ...
            # print(video_transform(buffer).shape,"123")
            return video_transform(buffer), torch.from_numpy(np.array(self.labels[index])).long(), self.fname[index]
...
if __name__ == '__main__':
    ...
    # train_dataloader = \
    #     DataLoader(VideoDataset_3_frames(mode='train'), batch_size=1, shuffle=False, num_workers=0)

    val_dataloader = \
        DataLoader(
            VideoDataset_3_frames(mode='train'),
    ...
    # print(len(test))
    for step, (buffer, label, ii) in enumerate(val_dataloader):
        print(buffer.shape)
        print(label)
